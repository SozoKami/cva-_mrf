...
class Model_params:
    ...
    def set_lgm_params(self):  # here params are constant
        ...
        lgm_params['mean rev'] = np.abs(
            np.random.normal(0.03, 2 / 300, self.nbr_currency))  # normal ranged from 1% to 5% using sigma 123 rule
        lgm_params['sigma'] = np.abs(
            np.random.normal(0.01, 2 / 300, self.nbr_currency))  # normal ranged from 0.5% to 2.5% using sigma 123 rule
        return lgm_params
...
def Expected_exposure_MC(t, portfolio, diffusion, ZC):
